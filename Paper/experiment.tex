...
\subsection{Base cases}\label{sec:basecases}
...
By base case here we mean a test run of the experiments \emph{without} music across multiple users, in order to determine an average measure for each experiment (we will discuss about the measures later in the \nameref{sec:measures} section).
...
For the first experiment (\testfirst) we set the BPM to 130, which is slightly faster than the base case which was 122 BPM).
...
We will discusses later in the \nameref{sec:conclusions} section how this difference impacted on the test results.
